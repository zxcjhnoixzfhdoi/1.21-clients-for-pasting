...
layout(location = 5) in float vRefractStrength;
...
void main() {
    ...
    float r = texture(blurTex, vUV + refractOffset * 1.00).r;
    float g = texture(blurTex, vUV + refractOffset * 0.85).g;
    float b = texture(blurTex, vUV + refractOffset * 0.70).b;
