...
public class WindChargeSimulation implements Imports {
    ...
    public WindChargeSimulation(Vec3 position, Vec3 velocity) {
        this(position, velocity, mc.player);
    }

    public WindChargeSimulation(Vec3 position, Vec3 velocity, Entity ignoredEntity) {
    ...
    private boolean canHitEntity(Entity entity) {
        if (!entity.canBeHitByProjectile() || entity.noPhysics) return false;
        if (entity instanceof AbstractWindCharge || entity.getType() == EntityType.END_CRYSTAL) return false;
