...
public class AimAssist extends Feature {
	...
	public AimAssist() {
		super(new FeatureInfo(
			Concat.of("Aim Assist"),
			Concat.of("Automatically aims at targets"),
			Category.COMBAT
		));
	}
	...
	private final BoolSetting onClickBoolSetting = new BoolSetting(Concat.of("On Click"), false);
	private final FloatSetting range = new FloatSetting(Concat.of("Range"), 6.0f, 1.0f, 50.0f, 0.1f);
	private final ModeSetting fovMode = new ModeSetting(Concat.of("FOV Mode"), Concat.of("Single"), Concat.of("Single"), Concat.of("Split"));
	private final FloatSetting fov = new FloatSetting(Concat.of("FOV"), 90.0f, 0.0f, 360.0f, 1f, o -> fovMode.is(Concat.of("Single")));
	private final FloatSetting yawFov = new FloatSetting(Concat.of("Yaw FOV"), 90.0f, 0.0f, 360.0f, 1f, o -> fovMode.is(Concat.of("Split")));
	private final FloatSetting pitchFov = new FloatSetting(Concat.of("Pitch FOV"), 90.0f, 0.0f, 180.0f, 1f, o -> fovMode.is(Concat.of("Split")));
	...
	private void onRender(MouseUpdateEvent event) {
		if (isNull() || hack.echo.client.api.MinecraftCompat.getScreen() != null) {
			RotationUtils.rotateBack(true, horizontalSpeed.getRandom(), verticalSpeed.getRandom(), getAimType(), false);
			return;
		}

		if (!isHoldingAllowed()) {
			// Dont disengage rots if maceswap is active...
			if (silent.getValue() &&
		...
		if (onClickBoolSetting.getValue() && !attackPressed) {
			disengageRotations();
			return;
		}

		// Find valid targets - filter by distance first (cheaper) before expensive FOV calculation
