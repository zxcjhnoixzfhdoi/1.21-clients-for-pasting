...
public class GlassIntSliderElement implements GlassSettingElement {
    ...
    private int lastSoundValue = Integer.MIN_VALUE;
    private boolean wasHovered = false;

    public GlassIntSliderElement(IntSetting setting) {
    ...
    public void render(Draw2D draw, float x, float y, Matrix4f mat, int mouseX, int mouseY, float delta) {
        ...
        CharSequence value = ClientUtil.intSequence(setting.getValue());
    ...
    private void updateValue(double mouseX) {
        ...
        int newVal = (int) (setting.getMinValue() + (setting.getMaxValue() - setting.getMinValue()) * percent);
        setting.setValueNoNotify(newVal);

        float range = setting.getMaxValue() - setting.getMinValue();
        float threshold = range * 0.03f;
        if (lastSoundValue == Integer.MIN_VALUE || Math.abs(newVal - lastSoundValue) >= threshold) {
    ...
    private float getProgress() {
        return (float) (setting.getValue() - setting.getMinValue()) / (setting.getMaxValue() - setting.getMinValue());
    }
}
