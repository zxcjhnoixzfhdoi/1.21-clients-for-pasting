...
public class GlassRangeElement implements GlassSettingElement {
    ...
    private final Animation animMin = new Animation(Easing.EASE_OUT_CUBIC, 150);
    private final Animation animMax = new Animation(Easing.EASE_OUT_CUBIC, 150);
    private float lastSoundValue = Float.MIN_VALUE;
    private boolean wasHovered = false;
    private boolean draggingUndecided = false;
    private float tieStartValue = Float.NaN;

    public GlassRangeElement(RangeSetting setting) {
        this.setting = setting;
        float minInit = setting.getMinPercentage();
        float maxInit = setting.getMaxPercentage();
        animMin.start(minInit, minInit);
        animMax.start(maxInit, maxInit);
    }
    ...
    public void render(Draw2D draw, float x, float y, Matrix4f mat, int mouseX, int mouseY, float delta) {
        this.x = x;
        this.y = y;
        if (draggingMin || draggingMax || draggingUndecided) {
            updateValue(mouseX);
        }

        animMin.updateTo(setting.getMinPercentage());
        animMax.updateTo(setting.getMaxPercentage());
        float minAnim = (float) animMin.getDelta();
        float maxAnim = (float) animMax.getDelta();
        ...
        CharSequence name = ClientUtil.truncateName(setting.getNameSequence(), 12);
        draw.text(Fonts.interSemiBold, mat, name, renderX + PADDING, y + 3, 6, argbMul(TEXT_ON_SURFACE_SECONDARY, delta));

        CharSequence value = ClientUtil.rangeSequence(setting.getMinValue(), setting.getMaxValue(), 1);
        ...
        float minX = trackStart + trackWidth * minAnim;
        float maxX = trackStart + trackWidth * maxAnim;
        draw.rect(mat, minX, sliderY, maxX - minX, 3, 1.5f, wa(SURFACE_CONTROL_ELEVATED.getRGB(), delta));

        draw.rect(mat, minX - 4, sliderY - 2, 8, 7, 4f, wa(CONTROL_KNOB.getRGB(), delta));
        draw.rect(mat, maxX - 4, sliderY - 2, 8, 7, 4f, wa(CONTROL_KNOB.getRGB(), delta));
    }
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        if (isInside(trackStart - 6, sliderY - 5, (trackEnd - trackStart) + 12, 12, (float) mouseX, (float) mouseY)) {
            ...
            float minDist = Math.abs(percent - setting.getMinPercentage());
            float maxDist = Math.abs(percent - setting.getMaxPercentage());

            if (minDist < maxDist) {
                draggingMin = true;
                lastSoundValue = setting.getMinValue();
            } else if (maxDist < minDist) {
                draggingMax = true;
                lastSoundValue = setting.getMaxValue();
            } else {
                draggingUndecided = true;
                tieStartValue = setting.getMinValue();
                lastSoundValue = tieStartValue;
            }
    ...
    public void mouseReleased(double mouseX, double mouseY, int button) {
        draggingMin = false;
        draggingMax = false;
        draggingUndecided = false;
        tieStartValue = Float.NaN;
    }
    ...
    private void updateValue(double mouseX) {
        ...
        float range = setting.getUpperBound() - setting.getLowerBound();
        float newValue = setting.getLowerBound() + range * percent;
        newValue = Math.round(newValue / setting.getIncrement()) * setting.getIncrement();

        if (draggingUndecided) {
            if (newValue <= setting.getMaxValue()) {
                setting.setMinValue(newValue);
            } else if (newValue >= setting.getMinValue()) {
        ...
        if (draggingMin) {
            ...
            if (newValue >= setting.getMinValue()) {
                setting.setMaxValue(newValue);
            }
        }

        float threshold = range * 0.03f;
        if (lastSoundValue == Float.MIN_VALUE || Math.abs(newValue - lastSoundValue) >= threshold) {
