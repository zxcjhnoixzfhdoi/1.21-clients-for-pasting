...
public class VkHueSlider extends VkShader {
    private static final int STRIDE = 84;
    private final SimpleUBO ubo;

    public VkHueSlider(SimpleUBO ubo) {
        super(STRIDE, 4);
        this.ubo = ubo;
    }
    ...
    protected VkGraphicsPipeline.Builder getPipelineBuilder() {
        return VkGraphicsPipeline.builder("2d/color/hue_slider.vert", "2d/color/hue_slider.frag")
                .stride(STRIDE)
    ...
    public void add(Matrix4f model, float x, float y, float w, float h) {
        ...
        MemUtil.putFloat(p + 80, Draw2D.nextZ());
        next();
    }
}
